...
const STAGES_DEPLOY = [
  {
    ...
    detect: (logs) =>
      logs.some((l) =>
        /Checkout|Cloning into|git clone|Fetching changes/i.test(l),
      ),
  },
  {
    key: "install",
    ...
    detect: (logs) =>
      logs.some((l) =>
        /Install Dependencies|npm ci|npm install|added \d+ package/i.test(l),
      ),
  },
  {
    key: "build",
    ...
    detect: (logs) =>
      logs.some((l) =>
        /\(Build\)|npm run build|Creating an optimized|compiled successfully/i.test(
          l,
        ),
      ),
  },
  {
    ...
    detect: (logs) =>
      logs.some((l) =>
        /\(Deploy\)|Deploying|rsync|robocopy|deployed to/i.test(l),
      ),
  },
];
...
const STAGES_TEST = [
  ...
  },
  {
    ...
    icon: FlaskConical,
    detect: (logs) =>
      logs.some((l) => /Run Tests|npm test|PASS |FAIL |Test Suites:/i.test(l)),
  },
  {
    key: "result",
    label: "Analyze",
    ...
    icon: Sparkles,
    detect: (logs, status) =>
      status === "SUCCESS" || status === "FAILED" || status === "UNSTABLE",
  },
];
...
const computeStages = (execution, stagesDef) => {
  ...
  return stagesDef.map((s, i) => {
    ...
    if (i === lastDetected + 1 && !isFinished)
      return { ...s, state: "pending-active" };
...
const StepIcon = ({ state, Icon }) => {
  ...
  if (state === "pending-active")
    return (
      <div className="w-4.5 h-4.5 rounded-full border-2 border-blue-500/50 bg-blue-500/10 flex items-center justify-center">
...
const stateConnector = (state) => {
  ...
  if (state === "active")
    return "bg-gradient-to-r from-green-500/60 to-blue-500/20 animate-pulse";
  return "bg-slate-700";
};
...
const AISummaryStrip = ({ aiSummary }) => {
  ...
  return (
    <div
      className={`mt-3 rounded-xl border p-3 ${colors[aiSummary.type] || colors.info}`}
    >
      ...
      {aiSummary.reason && (
        <p className="text-xs opacity-80 leading-relaxed">{aiSummary.reason}</p>
      )}
      {aiSummary.suggestion && (
        <p className="text-xs mt-2 font-medium border-t border-white/10 pt-2 opacity-90">
          {aiSummary.suggestion}
        </p>
      )}
    </div>
...
const BuildRoadmap = ({ executionId, action, onClose }) => {
  ...
  const stagesDef =
    (action || "").toUpperCase() === "TEST" ? STAGES_TEST : STAGES_DEPLOY;
  ...
  const isFinished =
    execution && ["SUCCESS", "FAILED", "UNSTABLE"].includes(execution.status);
  ...
  return (
    <div
      className={`rounded-2xl border overflow-hidden ${headerColors[execution?.status] || "border-blue-500/20 bg-blue-500/5"}`}
    >
      {/* Header */}
      <div className="px-5 py-3 border-b border-white/5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {isFinished && (
            <button
              onClick={onClose}
              className="text-gray-600 hover:text-gray-300 text-[10px] underline"
            >
              dismiss
            </button>
          )}
      ...
      {/* Compact Stepper: Fixed on mobile, part of layout on desktop */}
      <div className="hidden md:block px-5 py-4">
        <div className="flex flex-row relative gap-4 flex-nowrap">
          {stages.map((stage, i) => {
            ...
            return (
              <React.Fragment key={stage.key}>
                ...
                <div className="flex flex-col items-center gap-4 flex-1">
                  {/* Circle + icon */}
                  <div
                    className={`w-10 h-10 rounded-full border-2 flex items-center justify-center transition-all duration-500 z-10 shrink-0 ${
                      stage.state === "done"
                        ? "border-green-500/60 bg-green-500/10"
                        : stage.state === "unstable"
                          ? "border-yellow-500/60 bg-yellow-500/10"
                          : stage.state === "failed"
                            ? "border-red-500/60 bg-red-500/10"
                            : stage.state === "active"
                              ? "border-blue-500 bg-blue-500/15 shadow-[0_0_12px_rgba(59,130,246,0.4)]"
                              : stage.state === "pending-active"
                                ? "border-blue-500/30 bg-slate-800"
                  ...
                  <div className="text-center flex-1">
                    ...
                    <p
                      className={`text-[9px] mt-0.5 leading-tight ${stage.state === "active" ? "text-gray-400" : "text-gray-700"}`}
                    >
                      {stage.desc}
                    </p>
                  </div>
                ...
                {!isLast && (
                  <div
                    className={`w-0.5 h-20 transition-all duration-700 ${stateConnector(stage.state)}`}
                  />
                )}
              </React.Fragment>
            );
      ...
      {/* Mobile Stepper: Fixed bottom-right corner on small screens */}
      <div className="fixed bottom-4 right-4 md:hidden max-w-sm pointer-events-auto">
        <div className="flex flex-row relative gap-2 flex-wrap bg-slate-950/95 border border-slate-800 rounded-xl p-3 backdrop-blur">
          {stages.map((stage, i) => {
            const Icon = stage.icon;
            ...
            return (
              <div key={stage.key} className="flex flex-col items-center gap-1">
                {/* Circle + icon */}
                <div
                  className={`w-8 h-8 rounded-full border-2 flex items-center justify-center transition-all duration-500 z-10 shrink-0 ${
                    stage.state === "done"
                      ? "border-green-500/60 bg-green-500/10"
                      : stage.state === "unstable"
                        ? "border-yellow-500/60 bg-yellow-500/10"
                ...
                {/* Label */}
                <p className="text-[8px] text-gray-500 text-center w-12 truncate">
                  {stage.label}
                </p>
              </div>
            );
